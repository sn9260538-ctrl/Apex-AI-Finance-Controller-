...
const target2 = `<div className="flex justify-center">
        <button
          onClick={handleRun}
          disabled={isProcessing || (invoices.length === 0 && payments.length === 0)}
          className="px-10 py-4 bg-neu-primary shadow-[12px_12px_24px_rgba(163,177,198,0.5),-12px_-12px_24px_rgba(255,255,255,0.8)] hover:-translate-y-1 active:translate-y-1 rounded-full font-bold text-lg text-neu-base flex items-center gap-3 transition-all disabled:opacity-50"
...
const regex = /<div className="flex justify-center">\s*<button\s*onClick=\{handleRun\}\s*disabled=\{isProcessing \|\| \(invoices\.length === 0 && payments\.length === 0\)\}\s*className="px-10 py-4 bg-neu-primary shadow-\[12px_12px_24px_rgba\(163,177,198,0\.5\),-12px_-12px_24px_rgba\(255,255,255,0\.8\)\] hover:-translate-y-1 active:translate-y-1 rounded-full font-bold text-lg text-neu-base flex items-center gap-3 transition-all disabled:opacity-50"\s*>/;
